...
from Home Assistant rather than directly from the Nest device.
It tries ICMP first and uses a limited TCP reachability fallback when ICMP is
blocked. It publishes only names, private addresses, up/down state, latency,
the check method, and the Nest temperature.
